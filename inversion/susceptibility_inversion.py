...
#     and 3D magnetic susceptibility. Requires mesh, actv, and mrec if χ overlay is needed.
#     """

#     # --------------------------------------------------
#     # 1) Load and preprocess terrain DSM
#     # --------------------------------------------------
#     logging.info("Loading XYZ terrain...")
#     xyz = np.loadtxt(xyz_path, delimiter=",").astype(np.float32)

#     if DSM_BIN and DSM_BIN > 0:  # Grid thinning
...
#     elif 0 < PT_KEEP < 1.0:  # Random thinning
#         rng = np.random.default_rng(42)
#         idx = rng.choice(xyz.shape[0], size=int(xyz.shape[0]*PT_KEEP), replace=False)
#         xyz = xyz[np.sort(idx)]

#     x, y, z = xyz[:,0], xyz[:,1], xyz[:,2]
#     tri = Delaunay(np.c_[x, y])  # triangulate

#     # --------------------------------------------------
#     # 2) Load and downscale orthophoto
#     # --------------------------------------------------
#     logging.info("Loading orthophoto image...")
...
#         vertexcolor=vertex_rgb, opacity=TERRAIN_OPACITY_LEFT,
#         lighting=TERRAIN_LIGHT, flatshading=False, showscale=False, name="DSM + Image"
#     )
#     terrain_right = terrain_left.update(opacity=TERRAIN_OPACITY_RIGHT)

#     # --------------------------------------------------
#     # 3) Magnetic susceptibility model
#     # --------------------------------------------------
#     VAL_clip = VAL_left = np.array([])
#     vmin_full = vmax_full = iso_max_left = ISO_LEVEL_LEFT

#     if mesh is not None and actv is not None and mrec is not None:
#         logging.info("Preparing susceptibility model overlay...")
...
#         subplot_titles=(f"Terrain + χ ≥ {ISO_LEVEL_LEFT}", "Terrain + Full Susceptibility")
#     )

#     # Left plot: terrain + iso χ
...
#     # Right plot: terrain + full χ
#     fig.add_trace(terrain_right, row=1, col=2)
#     if FULL_MODE == "volume" and VAL_clip.size:
#         SENTINEL = -1e9
#         VAL_vol = np.where(np.isfinite(VAL_clip), VAL_clip, SENTINEL)
#         fig.add_trace(go.Volume(
#             x=X, y=Y, z=Z-50, value=VAL_vol,
#             isomin=vmin_full, isomax=vmax_full,
#             surface_count=25, opacity=0.08,
#             colorscale="Viridis",
#             showscale=True, colorbar=dict(title="χ (SI)")
#         ), row=1, col=2)
#     elif VAL_clip.size:
...
#         title="Side-by-side with Terrain Overlay (clipped to DSM surface)",
...
#     fig.update_scenes(camera=cam); fig.layout.scene2.camera = cam
...
def overlay_terrain_chi(
    ...
    if np.count_nonzero(valid_idx) >= 50:  # require enough points
        fig.add_trace(go.Isosurface(
            ...
            caps=dict(x_show=False, y_show=False, z_show=False),
            showscale=True,
            colorbar=dict(title="χ (SI)")
        ), row=1, col=1)
    else:
        logging.warning("Left graph too sparse for Isosurface, plotting as points instead")
